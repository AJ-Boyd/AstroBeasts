...
player_inventory_association = Table(
    "player_inventory",
    Base.metadata,
...
player_astrobeast_association = Table(
    "player_astrobeast",
    Base.metadata,
    Column("PlayerID", ForeignKey("Players.PlayerID"), primary_key=True),
...
class Player(Base):
    ...
    astroBeasts = relationship(
        "AstroBeast", secondary=player_astrobeast_association, back_populates="players"
    )
...
class PlayerInventory(Base):
    ...
    PlayerID = Column(
        Integer, ForeignKey("Players.PlayerID")
...
class AstroBeast(
    Base
):  # need to adjust to include relationship with stats. stats are not implemented in game yet.
    ...
    players = relationship(
        "Player",
        secondary="player_astrobeast_association",
        back_populates="astroBeasts",
    )

    # Relationship with players, assuming there is an association table for many-to-many relationship
    player_astrobeast_association = Table(
        "player_astrobeast_association",
        Base.metadata,
...
class AlienAttributesLink(Base):
    ...
    AliensID = Column(Integer, ForeignKey("Aliens.AliensID"), primary_key=True)
    AttributeID = Column(
        Integer, ForeignKey("AlienAttributes.AttributeID"), primary_key=True
    )
